...
try:
    ...
    # The FX/CPI reference file (raw/ref/makro_ceyreklik.csv) is not on this machine:
    # the transcription checks still run on nominal lira; conversion checks are skipped.
    MACRO = False
    ROWS = [
        {"donem": donem, "isletmeci": op, "tip": tip, "tl": tl}
        for tip, table in (("on_odemeli", mod.PREPAID), ("faturali", mod.POSTPAID))
        for donem, *values in table
        for op, tl in zip(mod.OPERATORS, values[:-1], strict=True)
    ]
needs_macro = pytest.mark.skipif(not MACRO, reason="makro_ceyreklik.csv yok")
