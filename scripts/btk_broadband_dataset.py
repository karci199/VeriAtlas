...
ANNUAL_REVENUE = {
    ...
    2023: 34_563_506_139,
    2024: 63_353_088_238,
    2025: 105_593_891_660,
}
QUARTERLY_REVENUE = {
    "2025-1": 21_151_569_648,
    "2025-2": 23_616_740_231,
    "2025-3": 28_774_264_523,
    "2025-4": 32_051_317_257,
    "2026-1": 33_475_278_067,
}
...
def write(rows):
    ...
    with open(out, "w", newline="", encoding="utf-8") as fh:
        w = csv.DictWriter(
            fh, fieldnames=(*FIELDS[:-1], "sabit_toplam", "kaynak_rapor")
        )
        w.writeheader()
        for row in rows:
...
def arpu(rows):
    ...
    year_end = {
        int(r["donem"][:4]): r["sabit_toplam"]
        for r in rows
        if r["donem"].endswith("-4")
    }
...
def write_arpu(rows):
    values = arpu(rows)
    with open(
        RAW / "sabit_genisbant_arpu.csv", "w", newline="", encoding="utf-8"
    ) as fh:
