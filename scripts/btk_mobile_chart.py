...
def load():
    ...
    for r in detail:
        ...
        row = {
            "d": r["donem"],
            "total": num("toplam"),
            "pre": num("on_odemeli_toplam"),
            "post": num("faturali_toplam"),
            "port": port.get(r["donem"]),
        }
        for op in ("turkcell", "vodafone", "ttmobil"):
            row[op] = {
                "total": num(op + "_toplam"),
                "pre": num(op + "_on_odemeli"),
...
def load_annual():
    ...
    rev = {
        int(r["yil"]): r
        for r in csv.DictReader(open(RAW / "sektor_gelir_reel.csv", encoding="utf-8"))
    }
    fixed = {
        int(r["yil"]): r
        for r in csv.DictReader(open(RAW / "sabit_abone.csv", encoding="utf-8"))
    }
    out = []
    for y in sorted(set(rev) | set(fixed)):
        ...
        out.append(
            {
                "d": str(y),
                "big4_tl": pick("buyuk4_tl"),
                "big4_usd": pick("buyuk4_usd"),
                "big4_real": pick("buyuk4_tl_2025"),
                "big4_usdreal": pick("buyuk4_usd_2025"),
                "sector_tl": pick("sektor_tl"),
                "sector_usd": pick("sektor_usd"),
...
def main():
    ...
    DEST.write_text(
        html.replace("__DATA__", json.dumps(load(), ensure_ascii=False)),
        encoding="utf-8",
    )
