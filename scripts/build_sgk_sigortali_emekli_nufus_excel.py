"""İl bazında sigortalı/nüfus, emekli/nüfus ve toplamı/nüfus — 2025, yan yana.

Aktif sigortalı (Tablo 1.7 toplamı) ve emekli (Tablo 2.5+2.19+2.32, malullük+yaşlılık,
ölüm aylığı hariç) aynı nüfus paydasına oranlanıyor. Üçüncü sütun ikisinin toplamı —
"nüfusun ne kadarı SGK sistemiyle bir şekilde bağlantılı" (ya prim ödüyor ya aylık
alıyor).

Run:  uv run python scripts/build_sgk_wage_excel2_combined.py
"""
...
TARGET = PUBLIC.parent / "cikti" / "analiz-sgk-sigortali-emekli-nufus-il-2025.xlsx"
...
def main() -> None:
        ...
        rows.append(
            {
                ...
                "emekli": emekli,
                "nufus": nufus,
                "sigortali_nufus": sigortali / nufus if nufus else None,
                "emekli_nufus": emekli / nufus if nufus else None,
                "toplam_nufus": (sigortali + emekli) / nufus if nufus else None,
            }
        )
    ...
    ranked_frame = ranked(
        frame.select(
            "il", "plaka", "sigortali", "emekli", "nufus",
            "sigortali_nufus", "emekli_nufus", "toplam_nufus",
        ),
        "toplam_nufus",
    )

    tr_s, tr_e, tr_n = frame["sigortali"].sum(), frame["emekli"].sum(), frame["nufus"].sum()
    tr = pl.DataFrame([{
        "sigortali": tr_s, "emekli": tr_e, "nufus": tr_n,
    ...
    headers = {
        ...
        "sigortali": "Aktif sigortalı", "emekli": "Emekli sayısı", "nufus": "Toplam nüfus",
        "sigortali_nufus": "Sigortalı / Nüfus", "emekli_nufus": "Emekli / Nüfus",
        "toplam_nufus": "(Sigortalı + Emekli) / Nüfus",
    }
    counts = ("sigortali", "emekli", "nufus")
    ...
    widths = {"il": 22, "plaka": 8, "sira": 7, "sigortali": 16, "emekli": 16, "nufus": 14}
    ...
    notes_sheet(
        book,
        [
            "İl bazında sigortalı/nüfus, emekli/nüfus ve toplamı/nüfus — 2025",
            "",
            "Sigortalı = aktif sigortalı, 4/1-a+4/1-b+4/1-c toplamı (Tablo 1.7).",
            "Emekli = malullük+yaşlılık aylığı alanlar, ölüm aylığı hariç",
            "(Tablo 2.5+2.19+2.32). Nüfus = TÜİK ADNKS toplam nüfus, 2025.",
            "",
            "Üçüncü sütun ikisinin toplamı — nüfusun ne kadarının SGK sistemiyle bir",
            "şekilde bağlantılı olduğu (ya prim ödüyor ya aylık alıyor). Geri kalan",
            "pay çocuklar, öğrenciler, ev hanımları, işsizler ve bakmakla yükümlü",
            "olunanlar — SGK'ya kaydı olmayan ya da yalnız sağlık güvencesi üzerinden",
            "bağlı olanlar.",
            "",
            "Sıralama '(Sigortalı+Emekli)/Nüfus' sütununa göre; her sütun ayrıca",
            "sıralanabilir.",
            "",
            "KAYNAK",
            "SGK İstatistik Yıllığı 2025, Tablo 1.7, 2.5, 2.19, 2.32 +",
            "population.csv.gz (TÜİK ADNKS).",
        ],
    )
    book.close()
    # endregion
