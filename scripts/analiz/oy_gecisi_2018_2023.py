...
R2018 = [("AKP", r"^AK PART"), ("CHP", r"^CHP$"), ("HDP", r"^HDP$"), ("MHP", r"^MHP$"),
         ("İYİ", r"^[İI]Y[İI]"), ("SP", r"^SAADET"), ("diğer18", None)]
R2023 = [("AKP", r"^AK PART"), ("CHP", r"^CHP$"), ("YSP", r"^YE[ŞS][İI]L SOL"), ("MHP", r"^MHP$"),
         ("İYİ", r"^[İI]Y[İI]"), ("YRP", r"^YEN[İI]DEN REFAH"), ("ZAFER", r"^ZAFER"), ("diğer23", None)]
...
def vec(rec, rules):
    ...
    for _, pat in rules:
        ...
        s = sum(n for c, n in v.items() if re.search(pat, c, re.I))
        out.append(s); kalan -= s
...
for key in ortak:
    ...
    if ka < 100 or kb < 100: atlanan += 1; continue
    if abs(kb / ka - 1) > 0.15: atlanan += 1; continue   # goc/sinir degisimi
    va, ta = vec(ra, R2018)
    vb, tb = vec(rb, R2023)
    va.append(max(ka - (ra.get("o") or 0), 0))   # katilmayan 2018
    vb.append(max(kb - (rb.get("o") or 0), 0))   # katilmayan 2023
    sa, sb = sum(va), sum(vb)
    if sa < 50 or sb < 50: atlanan += 1; continue
    X.append([x / sa for x in va]); Y.append([y / sb for y in vb]); W.append(ka)

X = np.array(X); Y = np.array(Y); W = np.array(W, dtype=float)
print("kullanilan mahalle:", len(X), "| atlanan:", atlanan, "| secmen:", f"{W.sum():,.0f}")
sw = np.sqrt(W)[:, None]
Xw, Yw = X * sw, Y * sw
...
print("\n2018 tabani (seçmen içinde %):", {rows[i]: round(100*w18[i],1) for i in range(len(rows))})
