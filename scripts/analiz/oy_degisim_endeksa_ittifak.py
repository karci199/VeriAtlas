...
for k in a:
    ...
    rows.append(
        dict(
            ...
            il=k[:5],
            secmen=rb["k"],
            d_cumhur=sh(rb, B["cumhur"]) - sh(ra, B["cumhur"]),
            d_millet=sh(rb, B["millet"]) - sh(ra, B["millet"]),
            d_emek=sh(rb, B["emek"]) - sh(ra, B["emek"]),
...
Z = np.column_stack(
    [np.log(X["nufus"].to_numpy()) if f == "nufus" else X[f].to_numpy() for f in feats]
).astype(float)
...
for s in range(5):
    ...
    for _ in range(100):
        ...
        C2 = np.array(
            [Z[lab == i].mean(0) if (lab == i).any() else C[i] for i in range(10)]
        )
        if np.allclose(C, C2):
            break
        C = C2
...
print(
    ...
    .agg(
        pl.len().alias("mah"),
        pl.col("secmen").sum(),
        pl.col("nufus").median().alias("nufus_med"),
        wm("universite"),
        wm("yasli"),
        wm("genc"),
        wm("evli"),
...
for t in ["d_cumhur", "d_millet", "d_emek"]:
    print(
        t,
        {
            v: round(df.select(pl.corr(t, v)).item(), 2)
            for v in [
                "universite",
                "dusuk_egitim",
                "yasli",
