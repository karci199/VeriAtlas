...
for k in a:
    ...
    rows.append(dict(id=k, ad=rb["ad"], il=k[:5], secmen=rb["k"],
        **{f"d_{p}":sh(rb,y)-sh(ra,x) for p,(x,y) in B.items()}, cumhur15=sh(ra,B["akp"][0]),
...
for t in ["d_akp","d_chp","d_mhp","d_hdp"]:
    print(t,{v:round(df.drop_nulls([t,v]).select(pl.corr(t,v)).item(),2) for v in ["universite","dusuk_egitim","yasli","genc","bekar","ses_ab","gelir","kira","cumhur15"]})
