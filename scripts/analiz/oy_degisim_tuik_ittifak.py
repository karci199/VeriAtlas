...
def swings(ra,rb): return {f"d_{k}":sh(rb,p)-sh(ra,p) for k,p in B.items()} | {"cumhur15":sh(ra,B["cumhur"])}
...
for var in ["nufus","buyume","cocuk","cocuk_degisim","secmen_artis","katilim_degisim","bekar","bosanmis","dul","cumhur15"]:
...
for var in ["uni","dusuk","cumhur15"]:
    ...
    print(f"## ILCE {var}")
    print(x.group_by("dilim").agg(pl.col(var).min().round(1).alias("alt"),pl.col(var).max().round(1).alias("ust"),wm("d_cumhur"),wm("d_millet"),wm("d_emek"),pl.len().alias("n")).sort("dilim"))
print("ILCE korelasyon", {t:{v:round(idf.select(pl.corr(t,v)).item(),2) for v in ["uni","dusuk","cumhur15"]} for t in ["d_cumhur","d_millet","d_emek"]})
