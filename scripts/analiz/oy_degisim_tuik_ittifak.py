...
b = load("mv2023")
B = {
    "cumhur": r"^(AK PART|MHP$|BBP$|B[ÜU]Y[ÜU]K B[İI]RL|YEN[İI]DEN REFAH|CUMHUR)",
...
def swings(ra, rb):
    return {f"d_{k}": sh(rb, p) - sh(ra, p) for k, p in B.items()} | {
...
names = {
    r["area_id"]: (r["parent_id"], fold(r["name_tr"]))
    for r in csv.DictReader(
        open("src/veriatlas/data/areas_tr_neighbourhoods.csv", encoding="utf-8")
    )
}
for r in csv.DictReader(
    open("src/veriatlas/data/areas_tr_villages.csv", encoding="utf-8")
):
    names[r["area_id"]] = (r["parent_id"], fold(r["name_tr"]))
pop = duckdb.sql(
    f"""select area_id, year(period_start) y, dims, value from '{F}' where indicator_id='population' and area_level in ('neighbourhood','village') and year(period_start) in (2015,2023)"""
).pl()
P = {}
for aid, y, d, v in pop.iter_rows():
    key = names.get(aid)
    if key:
        P.setdefault(key, {})[(y, d)] = v
med = {}
...
il_ad = {
    k: fold(v)
    for k, v in json.load(open(T + "il-adlari.json", encoding="utf-8")).items()
}
ilce_ad = {
    k: fold(v["ad"])
    for k, v in json.load(
        open(T + "secim-cb2023t1-ilce.json", encoding="utf-8")
    ).items()
}
rows = []
for k in a:
    if (
        k not in b
        or "~" in k
        or (a[k].get("k") or 0) < 300
        or (b[k].get("k") or 0) < 300
    ):
        continue
    p = P.get((k[:9], fold(b[k].get("ad", ""))))
    if not p:
        continue
    c15 = p.get((2015, "age=0-17"), 0)
    a15 = p.get((2015, "age=18+"), 0)
    c23 = p.get((2023, "age=0-17"), 0)
    a23 = p.get((2023, "age=18+"), 0)
    if not (c15 + a15) or not (c23 + a23):
        continue
    m = med.get(
        (il_ad.get(k[:5], ""), ilce_ad.get(k[:9], ""), fold(b[k].get("ad", "")))
    )
    rows.append(
        dict(
            id=k,
            ...
            **swings(a[k], b[k]),
            nufus=c23 + a23,
            buyume=100 * ((c23 + a23) / (c15 + a15) - 1),
            cocuk=100 * c23 / (c23 + a23),
            cocuk_degisim=100 * c23 / (c23 + a23) - 100 * c15 / (c15 + a15),
            secmen_artis=100 * (b[k]["k"] / a[k]["k"] - 1),
            katilim_degisim=100 * (b[k]["o"] / b[k]["k"] - a[k]["o"] / a[k]["k"]),
            bekar=float(m["hic_evlenmemis_oran"]) if m else None,
            bosanmis=float(m["bosanmis_oran"]) if m else None,
...
print(
    ...
    df.height,
    " medeni eslesen:",
    df.filter(pl.col("bekar").is_not_null()).height,
)
...
def wm(c):
    return (
        (
            (pl.col(c) * pl.col("secmen")).filter(pl.col(c).is_not_null()).sum()
...
for var in [
    ...
    x = df.filter(pl.col(var).is_not_null()).with_columns(
        (
            ((pl.col(var).rank("ordinal") - 1) * 10 / pl.len()).floor().cast(pl.Int32)
            + 1
        ).alias("dilim")
    )
    print(f"## {var}")
...
ia = json.load(open(T + "secim-mv2015k-ilce.json", encoding="utf-8"))
ib = json.load(open(T + "secim-mv2023-ilce.json", encoding="utf-8"))
edu = duckdb.sql(f"""
...
print(
    "edu dims ornek",
    duckdb.sql(
        f"select dims from '{F}' where indicator_id='education_level_district' limit 2"
    ).fetchall(),
)
...
for k in ia:
    if (
        ...
        and "-x" not in k
        and k in E
        and not ib[k].get("eski")
        and not ia[k].get("eski")
    ):
        ir.append(
            dict(
                id=k,
...
print(
    ...
    {
        t: {
            v: round(idf.select(pl.corr(t, v)).item(), 2)
            for v in ["uni", "dusuk", "cumhur15"]
        }
        for t in ["d_cumhur", "d_millet", "d_emek"]
    },
)
