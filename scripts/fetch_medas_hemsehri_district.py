"""Fetch where each district's residents are registered — hemsehrilik at district level.

Measure: "İkamet edilen ilçeye göre nüfusa kayıtlı olunan il". Rows are the districts of
one province (Düzey = İlçe, il = PROVINCE), columns the 81 provinces of registration, so
81 indicators × a province's districts — one year per query fits easily. The reverse
reading of the same square is `fetch_medas_diaspora_district.py`.

Raw files land in `raw/medas/hemsehri/` as `nufus-hemsehri-ilce-<il>-<yil>-<yil>.csv` and
are never overwritten. The province part of the name is Python's lower() of the Turkish
upper case (`AĞRI` → `ağri`), so an adapter reads the province from the file, not the name.

Run:  uv run python scripts/fetch_medas_hemsehri_district.py Bursa 2025
      (all provinces and years: C:/veri-ham/medas_ilce_kuyruk.py hemsehri)
"""
...
MEASURE_HINT = "kamet edilen ilçeye g"
...
def pick_level(page, level: str) -> None:
    """Düzey = İlçe; il kutusunda PROVINCE; ilçe listesinde başlık kutusuyla tümü."""
    for index in range(page.locator("select").count()):
        select = page.locator("select").nth(index)
        if not select.is_visible():
            continue
        options = select.locator("option").all_inner_texts()
        hit = [o for o in options if "lçe" in o]
        if hit:
            select.select_option(label=hit[0])
            settle(page, "duzey: " + hit[0])
            break
    for index in range(page.locator("select").count()):
        select = page.locator("select").nth(index)
        if not select.is_visible():
