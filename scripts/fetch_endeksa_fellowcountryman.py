"""Neighbourhood hemşehrilik (fellow-countryman) top-10 from Endeksa, mirrors fetch_endeksa_demography.py.

Endeksa's neighbourhood page shows a "KÜTÜK DAĞILIMI" chart -- the top 10 provinces its
residents are registered to, with counts -- that never showed up in the `/demography`
answer because it is a separate endpoint entirely, found by unpacking the site's own JS
bundle (main.region.min.js -> factory("fellowApi") -> CACHE + "/fellowcountryman").

    GET app.endeksa.com/fellowcountryman?countryId&cityId&countyId&districtId&level=3

Same AES-ECB ciphertext, same four required ids, same county-id lookup as demography.
Unlike MEDAS's hemşehrilik (81 provinces, 19 years, district level), this is a single
snapshot and only the top 10 -- but it is at *mahalle* level, which MEDAS never reaches.

Output: one file per district, `<data root>/endeksa/fellowcountryman/<area_id>.json`.

Run:  uv run python scripts/fetch_endeksa_fellowcountryman.py            # eksik olan her yer
      uv run python scripts/fetch_endeksa_fellowcountryman.py TR-16      # tek il
"""
...
OUT = HAM / "endeksa" / "fellowcountryman"
GEO = ROOT / "public" / "geo" / "neighbourhoods"
LOG = HAM / "endeksa" / "fellowcountryman.log"
WORKERS = 2
PAUSE = 0.25
...
API = "https://app.endeksa.com/fellowcountryman"


def fellows(client: httpx.Client, **params) -> list | None:
    """The decrypted [{CitizenCity, CountOf}, ...] answer, or None when empty."""
    response = client.get(API, params=params, timeout=30)
    response.raise_for_status()
    body = fg.decrypt(response.text)
    return body.get("FellowCountryman")
            ...
            def one(job, plate=plate, county_id=county_id, area_id=area_id):
                ...
                for attempt in range(3):
                    try:
                        ...
                        body = None
                    if body:
                        return district_id, {"name_tr": name, "fellowcountryman": body}
